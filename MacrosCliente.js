...
function SalvarCliente(Dados) {
    ...
    if (user.hasLock()) {
        ...
        let ultima_linha_com_dados = UltimaLinhaCom_dados_Da_Planilha(guia, "A"),
            linha
        let dadosPlan = guia.getRange("B6:" + "K" + ultima_linha_com_dados).getValues();
        ...
        let existeCidade
        dadosPlan = guiaCidades.getRange(2, 1, guiaCidades.getLastRow(), 2).getValues();

        for (linha = 0; linha < dadosPlan.length; linha++) {

            if (dadosPlan[linha][0] == Dados.Estado && dadosPlan[linha][1] == Dados.Cidade) {
...
function PesquisarCliente(nomeCliente) {
    ...
    for (var linha = 0; linha < dadosPlan.length; linha++) {
        ...
        if (dadosPlan[linha][1] == nomeCliente) {
            ...
            return ([Carregar.Cliente, Carregar.Cnpj, Carregar.Contato, Carregar.Rua, Carregar.Bairro, Carregar.Cidade, Carregar.Estado, Carregar.Obs])
...
function EditarCliente(Dados) {
    ...
    if (user.hasLock()) {
        ...
        var dadosClientes = guiaCliente.getRange("B6:" + "K" + ultima_linha_com_dados_cliente).getValues();
        var dadosPedidos = guiaPedido.getRange(2, 11, guiaPedido.getLastRow(), 1).getValues();
...
function VerificarCliente(nomeCliente) {
    ...
    var dadosPlan = guiaCliente.getRange(2, 2, guiaCliente.getLastRow(), 1).getValues();
...
function AtualizarListaClientes() {
    ...
    let guiaCliente = TabelaBanco("Clientes")
    let ultima_linha_com_dados_cliente = UltimaLinhaCom_dados_Da_Planilha(guiaCliente, "A") + 1
    let list = guiaCliente.getRange("B6:" + "K" + ultima_linha_com_dados_cliente).getValues();
